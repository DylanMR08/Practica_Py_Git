...
@personRoute.get("/")
async def index(dbConn: DBSession) -> List[PersonaValidator]:
    response = PersonaService.get_all(db=dbConn)
    return response

@personRoute.get("/{id}")
async def getById(id: int, dbConn: DBSession) -> PersonaValidator:
    response = PersonaService.get_by(id=id, db=dbConn)
    return response

@personRoute.delete("/{id}")
async def getById(id: int, dbConn: DBSession):
...
async def create(persona: PersonaValidator, dbConn: DBSession):
    ...
    return "Agregado correctamente" if response is True else "Error al agregar"
